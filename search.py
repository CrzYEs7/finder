...
def search_car_parts(user_inputs, user_id):
    """Search car parts"""
    ...
    base_query = """
        SELECT cp.*, pm.name AS manufacturer_name, fav.part_id as favorited,
            ...
            (CASE WHEN {} THEN 1 ELSE 0 END) +
            (CASE WHEN {} THEN 1 ELSE 0 END) +
            (CASE WHEN {} THEN 5 ELSE 0 END)) AS match_count
        FROM car_parts AS cp
        LEFT JOIN favorites_by_users AS fav ON cp.id = fav.part_id AND fav.user_id = ?
        LEFT JOIN car_parts_tags AS cpt ON cp.id = cpt.part_id
    ...
    base_query = base_query.format(
        conditions[fields[0]], conditions[fields[1]], conditions[fields[2]], conditions[fields[3]])
    ...
    # query input
    query_content_to_search = user_inputs * 4 + [user_id]
    print("query_content_to_search: ", query_content_to_search)

    # query
    res = cursor.execute(base_query, query_content_to_search)
